## 12.3.0 - 2026-04-24
### Added
* **`ExtractRequestConfig.ValidationMethod.ChunkCodeJaccardSimilarity`** — a new `"chunk_code_jaccard_similarity"` validation method that uses token-level Jaccard similarity between the source text chunk and a code description as a deterministic, non-LLM alternative to the existing LLM-based methods.
* **`ExtractRequestConfig.chunk_code_jaccard_similarity_filtering_threshold`** — a new optional `number` field (0.0–1.0) that sets the minimum Jaccard similarity score a code must reach to be considered valid when the `"chunk_code_jaccard_similarity"` validation method is active; ignored by all other methods.
