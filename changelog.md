...
- **Client renamed**: The main client class is now `PhenoMLClient` (was `phenomlClient`).
- **Sub-client classes renamed**: All resource client classes now use a `Client` suffix (e.g., `AgentClient` instead of `Agent`, `ConstrueClient` instead of `Construe`).
