## [17.5.0] - 2026-06-15
### Added
- **`client.fhir2Omop`** — new service client with a `create()` method that maps FHIR R4 resources or Bundles to OMOP CDM v5.4 rows via `POST /fhir2omop/create`.
- **`phenoml.fhir2Omop.OmopTables`** and typed row types (`PersonRow`, `VisitOccurrenceRow`, `ConditionOccurrenceRow`, `DrugExposureRow`, `ProcedureOccurrenceRow`, `MeasurementRow`, `ObservationRow`) — fully typed OMOP CDM v5.4 table output returned by `create()`.
- **`phenoml.fhir2Omop.MappingEntry`**, **`DroppedResource`**, and **`Summary`** — per-coding resolution report, dropped-resource list, and data-quality headline fields included in the `create()` response.
- **`phenoml.fhir2Omop` typed errors** (`BadRequestError`, `UnauthorizedError`, `InternalServerError`, `ServiceUnavailableError`) — thrown by `client.fhir2Omop.create()` for HTTP 400, 401, 500, and 503 responses respectively.
- **`Provider.Aidbox`** — new `"aidbox"` value added to the `fhirProvider` `Provider` enum; new `./fhir2Omop` and `./openapi.json` package subpath exports also added.
